...
{ for (size_t i = 0; i < seats.length(); i++) { if (seats[i] == 'x' && (bit & (1 << i))) return false; 
} return true; 
} 
...
{ 
	...
	for (int i = 0; i < width; i++) { if ((1 << i) & fbit) 
	{
		//앞자리에 앉는 녀석이 있을 때 
...
{ 
	...
	{ 
		...
		{
			int cnt = 0; 
			for (int i = 0; i < m; i++) { if ((1 << i) & bit) cnt++; }
	...
	{ 
		...
		for (pii bit : bits_set) {
			if (!seat_check(field[i], bit.first))
			continue; //부서진 자리에 앉으려고 하는 경우 
